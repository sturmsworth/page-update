...
class AdminTable extends Component {
  state = {
    data: [],
    headers: [
      { label: "First Name", key: "forms.applicantForm.applicantFirstName" },
      { label: "Middle Name", key: "forms.applicantForm.applicantMiddleName" },
      { label: "Last Name", key: "forms.applicantForm.applicantLastName" },
      {
        label: "Preferred Name",
        key: "forms.applicantForm.applicantPreferredName"
      },
      { label: "Address One", key: "forms.applicantForm.applicantAddressOne" },
      { label: "Address Two", key: "forms.applicantForm.applicantAddressTwo" },
      { label: "City", key: "forms.applicantForm.applicantCity" },
      { label: "State", key: "forms.applicantForm.applicantState" },
      { label: "Zip Code", key: "forms.applicantForm.applicantZipCode" },
      { label: "Submitted", key: "applicationSubmitted" }
    ],
    error: "",
    searchTerms: {
  ...
  searchData = (arrayName, query, parameter) => {
    ...
    return arrayName.filter((element, index) => {
      ...
      // District/Senator
      const isSenatorMatch = element.forms.miscForm.miscDistrict
        .toLowerCase()
        .includes(query.toLowerCase());
  ...
  handleShowOnlyNames = () => {
    this.setState({
      ...
      searchTerms: {
        ...this.state.searchTerms,
        applicantNames: !this.state.searchTerms.applicantNames
      }
    });
  };

  filterOnlyNames = () => {
    if (this.state.searchTerms.applicantNames) {
      return this.state.data.filter(element => {
        return (
          element.forms.applicantForm.applicantFirstName.length > 0 ||
          element.forms.applicantForm.applicantLastName.length > 0
        );
      });
    }
  };

  handleShowOnlySubmitted = () => {
    this.setState({
      ...this.state,
      searchTerms: {
        ...this.state.searchTerms,
        submitted: !this.state.searchTerms.submitted
      }
    });
  };

  filterOnlySubmitted = () => {
    if (this.state.searchTerms.submitted) {
      return this.state.data.filter(element => {
        return element.applicationSubmitted;
      });
    }
  };
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        columns: [
          {
            Header: "Submitted",
            accessor: "applicationSubmitted",
            Cell: row => {
              return (
                <span className={!row.value ? "text-danger" : "text-success"}>
                  {!row.value ? "No" : "Yes"}
                </span>
              );
            }
          ...
          {
            Header: "Terms Form",
            accessor: "formControl.termsCompleted",
            Cell: row => {
              return !row.value ? (
                <div>
                  <span
                    className="text-danger pr-5"
                    onClick={() => console.log("tee hee")}
                  >
          ...
          {
            Header: "Applicant Form",
            accessor: "formControl.applicantFormCompleted",
            Cell: row => {
              return (
                <div>
                  <span className={!row.value ? "text-danger" : "text-success"}>
          ...
          {
            Header: "Guardian Form",
            accessor: "formControl.guardianFormCompleted",
            Cell: row => {
              return !row.value ? (
                <span
          ...
          {
            Header: "Misc Form",
            accessor: "formControl.miscFormCompleted",
            Cell: row => {
              return !row.value ? (
                <span
                  className="text-danger"
          ...
          },
          {
            ...
            Cell: row => {
              return (
                <div>
                  <span
                    className={!row.value ? "text-danger" : "text-success"}
                    onClick={() => {
                      this.setState({
                        ...this.state,
                        popupThree: true,
          ...
          },
          {
            ...
            Cell: row => {
              return !row.value ? (
                <span>
                  {row.original.applicationSubmitted ? (
                    <button
                      className="btn btn-success"
                      onClick={() => {
                        const findItem = this.state.data.find(
                          item =>
                            item.userInfo.email === row.original.userInfo.email
                        );
                        ...
                        if (row.original.applicationSubmitted) {
                          const batch = db.batch();

                          const userRef = db
                            .collection("users")
                            .doc(row.original.userInfo.email);

                          batch.update(userRef, { adminVerification: true });
                          ...
                          this.setState({
                            ...this.state,
                            adminChangeMessages:
                              "Cannot verify an application that has not been submitted."
                          });
                        }
                      }}
                    >
                    ...
                    <button className="btn btn-disabled">Verify</button>
                  )}
                </span>
              ) : (
                <span>
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (row.original.applicationSubmitted) {
                        ...
                        batch.update(userRef, { adminVerification: false });
                        ...
                        this.setState({
                          ...this.state,
                          adminChangeMessages:
                            "Cannot revoke an application that has not been submitted."
                        });
                      }
                    }}
                  >
    ...
    if (searchTerms.firstName) {
      searchData = this.searchData(data, searchTerms.firstName, "firstName");
      table = (
        <ReactTable
          data={searchData}
          columns={columns}
      ...
      searchData = this.searchData(data, searchTerms.lastName, "lastName");
      table = (
        <ReactTable
          data={searchData}
          columns={columns}
      ...
      searchData = this.searchData(data, searchTerms.district, "district");
      table = (
        <ReactTable
          data={searchData}
          columns={columns}
      ...
      searchData = this.searchData(data, searchTerms.email, "email");
      table = (
        <ReactTable
          data={searchData}
          columns={columns}
              ...
              <div className="form-check pl-0">
                <span className="pr-2">
                  <input
                    type="checkbox"
                    id="showNames"
                    onChange={this.handleShowOnlyNames}
                    value={this.state.searchTerms.applicantNames}
                  />
                </span>
                <span>
              ...
              <div className="form-check pl-0">
                <span className="pr-2">
                  <input
                    type="checkbox"
                    id="showNames"
                    value={this.state.searchTerms.submitted}
                    onChange={this.handleShowOnlySubmitted}
                  />
                </span>
                <span>
                  <label htmlFor="showNames">Show Submitted</label>
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="row text-center pt-3">
          <div className="col">
            <CSVLink
              data={data}
              headers={headers}
              filename={`${new Date().getFullYear()}-page-report.csv`}
